...
![1301637312269_.pic_hd_副本](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20211119170012.jpg)
...
- 我们刚刚讲到，name 是必填的，所以，我们把它放到构造函数中，强制创建对象的时候就设置。如果必填的配置项有很多，把这些必填配置项都放到构造函数中设置，那**构造函数就又会出现参数列表很长**的问题。如果我们把必填项也通过 set() 方法设置，那**校验这些必填项是否已经填写的逻辑**就无处安放了。
...
| 应用场景               | 建造者模式实现                                               | 作用                                            |
| ---------------------- | ------------------------------------------------------------ | ----------------------------------------------- |
| 类的参数较多           | 1. 业务类：<br/>定义属性<br/>getter()方法                    | 创建一种类型的复杂对象<br/>定制化创建不同的对象 |
| 类的属性之前有依赖关系 | 2. 内部静态类Builder：<br/>copy业务类属性<br/>各属性set()方法返回builder<br/>build()方法返回业务类，并统一校验 |                                                 |
| 创建不可变对象         | 3. 构造函数private                                           |                                                 |

和工厂模式进行比较：

| 比较 | 建造者模式                                       | 工厂模式                                                     |
| ---- | ------------------------------------------------ | ------------------------------------------------------------ |
| 作用 | 创建一种类型的复杂对象                           | 创建不同但是相关类型的对象（继承同一父类或者接口的一组子类） |
| 原理 | 通过设置不同的可选参数，“定制化”地创建不同的对象 | 由给定的参数来决定创建哪种类型的对象                         |
