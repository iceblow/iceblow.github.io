---
layout: post
title: 【阿里技术】一文搞懂Redis
categories: [数据库,中间件]
description: some word here
keywords: keyword1, keyword2
---

转载阿里技术微信公众号的《一文搞懂redis》，篇幅较大，[原文链接](https://mp.weixin.qq.com/s/7ct-mvSIaT3o4-tsMaKRWA)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324093647.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324093956.png)
...
1) 避免缓冲区溢出：进行两个字符串拼接c语言可使用strcat函数，但如果没有足够的内存空间。就会造成缓冲区溢出；而用sds在进行合并时会先用len检查内存空间是否满足需求，如果不满足，进行空间扩展，不会造成缓冲区溢出；
1) 减少修改字符串的内存重新分配次数：c语言字符串不记录字符串长度，如果要修改字符串要重新分配内存，如果不进行重新分配会造成内存缓冲区泄露；
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324094440.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324094517.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324094659.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324094747.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324094911.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095008.png)

如果想查找到node5需要从node1查到node5，查询耗时

但如果在node上加上索引：

![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095021.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095035.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095326.png)
...
```
# 时间策略   save m n m秒内修改n次key，触发rdbsave 900 1save 300 10save 60 10000
# 文件名称dbfilename dump.rdb
# 文件保存路径dir /home/work/app/redis/data/
# 如果持久化出错，主进程是否停止写入stop-writes-on-bgsave-error yes
# 是否压缩rdbcompression yes
...
```
# 默认不开启aof  而是使用rdb的方式appendonly no
# 默认文件名appendfilename "appendonly.aof"
# 每次修改都会sync 消耗性能# appendfsync always# 每秒执行一次 sync 可能会丢失这一秒的数据appendfsync everysec# 不执行 sync ,这时候操作系统自己同步数据，速度最快# appendfsync no 
```
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095630.png)

#### **发布**

![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095709.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095855.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095928.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324095944.png)
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324100116.png)

订阅

当有信的订阅时，会将订阅的客户端和模式信息添加到链表后面。

![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324100133.png)
...
2.前者称为主节点(master)，后者称为从节点(slave)；数据的复制是单向的，只能由主节点到从节点默认情况下，每台redis服务器都是主节点；且一个主节点可以有多个从节点（或者没有），但一个从节点只有一个主
```
...
![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324100244.png)
...
下图是一个典型的哨兵集群监控的逻辑图
 ...
 ![图片](https://img-1257951221.cos.ap-shanghai.myqcloud.com//20220324100354.png)
